...
<div class="row">
	<ol class="breadcrumb">
		...
		<li>
			<i class="fa fa-group"></i> Nhóm quản trị
		</li>
		<a class="btn-add-new" href="?<?php echo TTH_PATH?>=core_role_add">Thêm nhóm</a>
	</ol>
</div>
<!-- /.row -->
...
if(isset($_POST['idDel'])){
	...
	$db->delete();
	loadPageSucces("Đã xóa Nhóm thành công.","?".TTH_PATH."=core_role");
}
?>
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default panel-no-border">
			<div class="table-responsive">
				<form action="?<?php echo TTH_PATH?>=core_role" method="post" id="deleteArt">
						...
						foreach($rows as $row) {
							...
							<tr>
								...
								<td align="center">
									<?php
									$db->table = "core_user";
									$db->condition = "role_id = ".($row["role_id"]+0);
									$db->order = "";
									$db->limit = "";
									$db->select();
									echo formatNumberVN($db->RowCount+0);
									?>
								</td>
								...
								<td class="details-control" align="center">
									<div class="checkbox">
										<a href="?<?php echo TTH_PATH?>=core_role_edit&id=<?php echo $row['role_id']?>"><img data-toggle="tooltip" data-placement="top" title="Chỉnh sửa" src="images/edit.png"></a> &nbsp;
										<label class="checkbox-inline">
											<input type="checkbox" data-toggle="tooltip" data-placement="top" title="Xóa" class="checkboxArt" name="idDel[]" value="<?php echo $row['role_id']?>">
										</label>&nbsp;&nbsp;
										<a href="?<?php echo TTH_PATH?>=core_dashboard&id=<?php echo $row['role_id']?>"><img data-toggle="tooltip" data-placement="top" title="Quyền quản trị" src="images/list.png"></a>
									</div>
								</td>
							</tr>
						<?php
					...
					<div class="row">
						<div class="col-sm-6"><?php echo showPageNavigation($page, $totalpages, '?'.TTH_PATH.'=core_role&page=')?></div>
