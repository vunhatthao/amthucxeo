...
<div class="row">
	<ol class="breadcrumb">
		...
		<li>
			<i class="fa fa-list"></i> <?php echo getNameMenu($document_menu_id, 'document')?>
		</li>
		<a class="btn-add-new" href="?<?php echo TTH_PATH?>=document_add&id=<?php echo $document_menu_id?>">Thêm tệp tin</a>
	</ol>
</div>
<!-- /.row -->
<?php echo dashboardCoreAdminTwo("document_list;".$category_id_core)?>
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default panel-no-border">
			<div class="table-responsive">
				<form action="?<?php echo TTH_PATH?>=document_list" method="post" id="deleteArt">
					<input type="hidden" name="list_id" value="<?php echo $document_menu_id?>" />
					<table class="table display table-manager" cellspacing="0" cellpadding="0" id="dataTablesList">
						...
						foreach($rows as $row) {
							...
							<tr>
								...
								<td align="center"><?php echo stripslashes($row['type'])?></td>
								<?php
								//-----------Kiểm tra phân quyền ---------------------------------------
								...
								<td class="details-control" align="center">
									<div class="checkbox">
										...
										<label class="checkbox-inline">
											<input type="checkbox" data-toggle="tooltip" data-placement="top" title="Xóa" class="checkboxArt" name="idDel[]" value="<?php echo $row['document_id']?>">
										</label>
									</div>
								</td>
							</tr>
					...
					<div class="row">
						<div class="col-sm-6"><?php echo showPageNavigation($page, $totalpages, '?'.TTH_PATH.'=document_list&id='.$document_menu_id.'&page=')?></div>
						<div class="col-sm-6" align="right" style="padding: 7px 0;">
							...
							<input type="button" class="btn btn-primary btn-xs <?php echo in_array("document_del;".$category_id_core,$corePrivilegeSlug)? "confirmManager" : "alertManager"?> " value="Xóa" name="deleteArt">
						</div>
					</div>
				</form>
			</div>
