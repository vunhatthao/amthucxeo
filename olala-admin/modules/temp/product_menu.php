...
//
function productMenu($act, $typeFunc, $product_menu_id, $category_id, $name, $slug, $plus, $title, $description, $keywords, $parent, $is_active, $hot, $img, $error) {
	global $tth, $db;
			...
			<div class="panel-body">
				<form action="<?php echo $act?>" method="post" enctype="multipart/form-data">
					...
					<table class="table table-hover" style="width: 70%;">
						...
						<tr>
							<td align="right"><label>Tên thu gọn:</label></td>
							<td><input class="form-control" type="text" name="plus" maxlength="255" value="<?php echo stripslashes($plus)?>"></td>
						</tr>
						<tr>
...
<script>
	$('.file-img').fileinput({
		<?php if($img!='no' && $img!='') { ?>
		initialPreview: [
			"<img src='../uploads/product_menu/<?php echo $img?>' class='file-preview-image' title='<?php echo $img?>' alt='<?php echo $img?>'>"
		],
		<?php } ?>
