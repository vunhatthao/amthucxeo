...
function gallery($act, $typeFunc, $gallery_id, $gallery_menu_id, $name, $slug, $img, $comment, $content, $link, $is_active, $hot, $created_time, $upload_img_id, $error) {
	global $db, $tth;
	$category_id = 0;
				...
				<form action="<?php echo $act?>" method="post" enctype="multipart/form-data">
					...
					<table class="table table-hover" style="width: 80%;">
						...
						<tr>
							...
							<td><input class="form-control file file-img" type="file" name="img" data-show-upload="false" data-max-file-count="1" accept="image/*" <?php echo ($typeFunc=='add') ? 'required="required"' : ''?> ></td>
						</tr>
						<tr>
							<?php if($category_id == 91){echo '<td align="right"><label>Bổ sung:</label></td>';} else echo '<td align="right"><label>Liên kết:</label></td>';?>
							<td><input class="form-control" type="text" name="link" value="<?php echo stripslashes($link)?>" ></td>
						</tr>
						<tr>
							<td align="right" class="ver-top"><label>Mô tả:</label></td>
							<td><textarea class="form-control" rows="3" name="comment" ><?php echo stripslashes($comment)?></textarea></td>
						</tr>
						<tr>
							<td align="right" class="ver-top"><label>Upload album:</label></td>
							<td><input id="album" class="form-control file" type="file" name="images[]" data-max-file-count="18" accept="image/*" multiple></td>
						</tr>
						<tr>
