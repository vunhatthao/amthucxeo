...
<div class="row">
	<ol class="breadcrumb">
		...
		<li>
			<a href="?<?php echo TTH_PATH?>=tour_manager"><i class="fa fa-edit"></i> Quản lý nội dung</a>
		</li>
		<li>
			<a href="?<?php echo TTH_PATH?>=tour_manager"><i class="fa fa-globe"></i> Tour du lịch</a>
		</li>
		<li>
			<i class="fa fa-list"></i> <?php echo getNameMenu($tour_menu_id, 'tour')?>
		</li>
		<a class="btn-add-new" href="?<?php echo TTH_PATH?>=tour_add&id=<?php echo $tour_menu_id?>">Thêm tour</a>
	</ol>
</div>
<!-- /.row -->
<?php echo dashboardCoreAdminTwo("tour_list;".$category_id_core)?>
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default panel-no-border">
			<div class="table-responsive">
				<form action="?<?php echo TTH_PATH?>=tour_list" method="post" enctype="multipart/form-data" id="deleteArt">
					<input type="hidden" name="list_id" value="<?php echo $tour_menu_id?>" />
					<table class="table display table-manager" cellspacing="0" cellpadding="0" id="dataTablesList">
						...
						foreach($rows as $row) {
							...
							<tr>
								...
								<td><?php echo stripslashes($row['name'])?><?php echo ($row['tour_keys']=='') ? ''  : '&nbsp; (Mã: <span style="font-weight: bold; color: #ff0000;">'.stripslashes($row['tour_keys']) .'</span>)' ?></td>
								<?php
								//-----------Kiểm tra phân quyền ---------------------------------------
								...
								<td class="details-control" align="center">
									<div class="checkbox">
										<a href="?<?php echo TTH_PATH?>=tour_edit&id=<?php echo $row['tour_id']?>"><img data-toggle="tooltip" data-placement="top" title="Chỉnh sửa" src="images/edit.png"></a>&nbsp;&nbsp;
										<label class="checkbox-inline">
											<input type="checkbox" data-toggle="tooltip" data-placement="top" title="Xóa" class="checkboxArt" name="idDel[]" value="<?php echo $row['tour_id']?>">
										</label>
									</div>
								</td>
							</tr>
					...
					<div class="row">
						<div class="col-sm-6"><?php echo showPageNavigation($page, $totalpages,'?'.TTH_PATH.'=tour_list&id='.$tour_menu_id.'&page=')?></div>
						<div class="col-sm-6" align="right" style="padding: 7px 0;">
							...
							<input type="button" class="btn btn-primary btn-xs <?php echo in_array("tour_del;".$category_id_core,$corePrivilegeSlug)? "confirmManager" : "alertManager"?> " value="Xóa" name="deleteArt">
						</div>
					</div>
				</form>
			</div>
