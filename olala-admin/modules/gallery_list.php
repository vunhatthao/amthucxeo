...
if(isset($_POST['idDel'])){
	...
	// Xóa Tb_ link
	$db->table = "link";
	$db->condition = "`category` = $category_id AND `menu` = $gallery_menu_id AND `post` IN ($idDel)";;
	$db->delete();
	// Xóa csld hình ảnh.
	$db->table = "gallery";
	$db->condition = "`gallery_id` IN ($idDel)";
	$db->delete();
	loadPageSucces("Đã xóa dữ liệu thành công.", "?".TTH_PATH."=gallery_list&id=" . $gallery_menu_id);
}
?>
<!-- Menu path -->
<div class="row">
	<ol class="breadcrumb">
		...
		<li>
			<i class="fa fa-list"></i> <?php echo getNameMenuGal($gallery_menu_id)?>
		</li>
		<?php
		if($category_id==91) { ?>
			<a class="btn-add-new" href="?<?php echo TTH_PATH?>=video_add&id=<?php echo $gallery_menu_id?>">Thêm video clip</a>
		<?php } else { ?>
			<a class="btn-add-new" href="?<?php echo TTH_PATH?>=gallery_add&id=<?php echo $gallery_menu_id?>">Thêm hình ảnh</a>
		<?php } ?>
	</ol>
</div>
<!-- /.row -->
<?php echo dashboardCoreAdminTwo("gallery_list;".$category_id)?>
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default panel-no-border">
			<div class="table-responsive">
				<form method="post" id="deleteArt">
					<table class="table display table-manager" cellspacing="0" cellpadding="0" id="dataTablesList">
						...
						foreach($rows as $row) {
							...
							<tr>
								...
								<?php
								//-----------Kiểm tra phân quyền ---------------------------------------
								if(in_array("gallery_edit;".$category_id,$corePrivilegeSlug)) {
								?>
								<td align="center">
								...
								<td class="details-control" align="center">
									<div class="checkbox">
										<?php
										$href = ($category_id==91) ? '?'.TTH_PATH.'=video_edit&id='.$row['gallery_id'] : '?'.TTH_PATH.'=gallery_edit&id='.$row['gallery_id'];
										?>
										<a href="<?php echo $href?>"><img data-toggle="tooltip" data-placement="top" title="Chỉnh sửa" src="images/edit.png"></a> &nbsp;
										<label class="checkbox-inline">
											<input type="checkbox" data-toggle="tooltip" data-placement="top" title="Xóa" class="checkboxArt" name="idDel[]" value="<?php echo $row['gallery_id']?>">
										</label>
									</div>
								</td>
							</tr>
					...
					<div class="row">
						<div class="col-sm-6"><?php echo showPageNavigation($page, $totalpages, '?'.TTH_PATH.'=gallery_list&id='.$gallery_menu_id.'&page=')?></div>
						<div class="col-sm-6" align="right" style="padding: 7px 0;">
							...
							<input type="button" class="btn btn-primary btn-xs <?php echo in_array("gallery_del;".$category_id,$corePrivilegeSlug)? "confirmManager" : "alertManager"?> " value="Xóa" name="deleteArt">
						</div>
					</div>
				</form>
			</div>
