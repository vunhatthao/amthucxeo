...
if($del != 0) {
	...
	$rows = $db->select();
	foreach($rows as $row) {
		if(!empty($row['img']) && glob($dir_dest . DS .'car_menu'. DS . '*'.$row['img'])) {
			array_map("unlink", glob($dir_dest . DS .'car_menu'. DS . '*'.$row['img']));
		}
	}
	...
	loadPageSucces("Đã xóa Chuyên mục thành công.", "?".TTH_PATH."=car_manager");
				...
				<table class="table table-manager table-hover">
					...
					foreach($rows as $row) {
						...
						<tr class="category">
							...
							<td align="center">
								<a href="?<?php echo TTH_PATH?>=car_menu_add&id_cat=<?php echo $row["category_id"]?>"><img data-toggle="tooltip" data-placement="left" title="Thêm mục" src="images/add.png"></a>
								&nbsp;
...
function loadMenuCategory($db, $level, $parent, $category_id){
	...
	foreach($rows2 as $row) {
		...
		<tr>
			...
			if(in_array("car_menu_edit;".$row["category_id"],$corePrivilegeSlug)) {
				?>
				<td align="right">
					<?php echo showSort("sort_".$row["car_menu_id"]."", $countList,$row["sort"], $width, 0, $row["car_menu_id"] ,'car_menu', 1);?>
				</td>
				<td align="center">
					<?php echo ($row["is_active"]+0==0)?
						'<div class="btn-event-close" data-toggle="tooltip" data-placement="top" title="Mở" onclick="edit_status($(this), '.$row["car_menu_id"].', \'is_active\', \'car_menu\');" rel="1"></div>'
						:
						'<div class="btn-event-open" data-toggle="tooltip" data-placement="top" title="Đóng" onclick="edit_status($(this), '.$row["car_menu_id"].', \'is_active\', \'car_menu\');" rel="0"></div>'
					?>
				</td>
				<td align="center">
					<?php echo ($row["hot"]+0==0)?
						'<div class="btn-event-close" data-toggle="tooltip" data-placement="top" title="Mở" onclick="edit_status($(this), '.$row["car_menu_id"].', \'hot\', \'car_menu\');" rel="1"></div>'
						:
						'<div class="btn-event-open" data-toggle="tooltip" data-placement="top" title="Đóng" onclick="edit_status($(this), '.$row["car_menu_id"].', \'hot\', \'car_menu\');" rel="0"></div>'
					?>
				</td>
			<?php
			}
				...
				?>
				<td align="right">
					<?php echo showSort("sort_".$row["car_menu_id"]."", $countList,$row["sort"], $width, 0, $row["car_menu_id"] ,'car_menu', 0);?>
				</td>
				<td align="center">
					<?php echo ($row["is_active"]+0==0)?
						'<div class="btn-event-close alertManager" data-toggle="tooltip" data-placement="top" title="Mở"></div>'
			...
			<td align="center">
				<?php echo ($row["img"]=='no')?
					'<img data-toggle="tooltip" data-placement="top" title="Không có hình" src="images/error.png">'
					:
					'<img id="popover-'.$row["car_menu_id"].'" class="btn-popover" title="'.stripslashes($row["name"]).'" src="images/OK.png">
					<script>
							var image = \'<img src="../uploads/car_menu/'.$row["img"].'">\';
							$(\'#popover-'.$row["car_menu_id"].'\').popover({placement: \'bottom\', content: image, html: true});
					</script>'
				?>
			</td>
			<td align="center">
				...
				if ($level < 30){
					?>
					<a href="?<?php echo TTH_PATH?>=car_menu_add&id_cat=<?php echo $row["category_id"]?>&id_art=<?php echo $row["car_menu_id"]?>"><img data-toggle="tooltip" data-placement="left" title="Thêm mục" src="images/add.png"></a>
					&nbsp;
				<?php } else { ?>
					<span style="width: 16px; height: 1px; display: inline-block;""></span>
					&nbsp;
				<?php } ?>
				<a href="?<?php echo TTH_PATH?>=car_menu_edit&id=<?php echo $row["car_menu_id"]?>"><img data-toggle="tooltip" data-placement="top" title="Chỉnh sửa" src="images/edit.png"></a>
				&nbsp;
				<?php
				if(!in_array("car_menu_del;".$row["category_id"],$corePrivilegeSlug)) {
					?>
				...
				<?php
					...
					?>
					<a class="confirmManager" style="cursor: pointer;" id="?<?php echo TTH_PATH?>=car_manager&del=<?php echo $row["car_menu_id"]?>"><img data-toggle="tooltip" data-placement="right" title="Xóa mục" src="images/remove.png"></a>
				<?php }
				//----------- end if ------------
				?>
			</td>
			<td align="center">
				<a href="?<?php echo TTH_PATH?>=car_list&id=<?php echo $row['car_menu_id']?>"><img data-toggle="tooltip" data-placement="top" title="Danh sách xe" src="images/list.png"></a>
			</td>
			<?php
			if ($level < 30){
				loadMenuCategory($db, $level+30, $row["car_menu_id"]+0, $row["category_id"]+0);
			}
			?>
		</tr>
	<?php
