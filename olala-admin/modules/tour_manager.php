				...
				<table class="table table-manager table-hover">
					...
					foreach($rows as $row) {
						...
						<tr class="category">
							...
							<td align="center">
								<a href="?<?php echo TTH_PATH?>=tour_menu_add&id_cat=<?php echo $row["category_id"]?>"><img data-toggle="tooltip" data-placement="left" title="Thêm mục" src="images/add.png"></a>
								&nbsp;
	...
	foreach($rows2 as $row) {
		...
		<tr>
			...
			if(in_array("tour_menu_edit;".$row["category_id"],$corePrivilegeSlug)) {
				?>
				<td align="right">
					<?php echo showSort("sort_".$row["tour_menu_id"]."", $countList,$row["sort"], $width, 0, $row["tour_menu_id"] ,'tour_menu', 1);?>
				</td>
				<td align="center">
					<?php echo ($row["is_active"]+0==0)?
				...
				?>
				<td align="right">
					<?php echo showSort("sort_".$row["tour_menu_id"]."", $countList,$row["sort"], $width, 0, $row["tour_menu_id"] ,'tour_menu', 0);?>
				</td>
				<td align="center">
					<?php echo ($row["is_active"]+0==0)?
							'<div class="btn-event-close alertManager" data-toggle="tooltip" data-placement="top" title="Mở"></div>'
			...
			<td align="center">
				...
				if ($level < 20){
					?>
					<a href="?<?php echo TTH_PATH?>=tour_menu_add&id_cat=<?php echo $row["category_id"]?>&id_art=<?php echo $row["tour_menu_id"]?>"><img data-toggle="tooltip" data-placement="left" title="Thêm mục" src="images/add.png"></a>
					&nbsp;
				<?php } else { ?>
					<span style="width: 16px; height: 1px; display: inline-block;""></span>
					&nbsp;
				<?php } ?>
				<a href="?<?php echo TTH_PATH?>=tour_menu_edit&id=<?php echo $row["tour_menu_id"]?>"><img data-toggle="tooltip" data-placement="top" title="Chỉnh sửa" src="images/edit.png"></a>
				&nbsp;
				<?php
				...
				<?php
					...
					?>
					<a class="confirmManager" style="cursor: pointer;" id="?<?php echo TTH_PATH?>=tour_manager&del=<?php echo $row["tour_menu_id"]?>"><img data-toggle="tooltip" data-placement="right" title="Xóa mục" src="images/remove.png"></a>
				<?php }
				//----------- end if ------------
				?>
			</td>
			<td align="center">
				<a href="?<?php echo TTH_PATH?>=tour_list&id=<?php echo $row['tour_menu_id']?>"><img data-toggle="tooltip" data-placement="top" title="Danh sách bài viết" src="images/list.png"></a>
			</td>
			<?php
			if ($level < 20){
