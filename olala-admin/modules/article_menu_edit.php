	...
	if(empty($name)) $error = '<span class="show-error">Vui lòng nhập tên chuyên mục.</span>';
		...
		if($OK) {
			...
			if($handleUploadImg) {
				...
				$imgUp->file_new_name_body    = 'banner-' . $img_name_file;
				$imgUp->image_resize          = true;
				$imgUp->image_ratio_crop      = true;
				$imgUp->image_x               = 1920;
				$imgUp->image_y               = 456;
				...
				$imgUp->file_new_name_body    = 'im-' . $img_name_file;
				$imgUp->image_resize          = true;
				$imgUp->image_ratio_crop      = true;
				$imgUp->image_x               = 525;
				$imgUp->image_y               = 350;
...
if(!$OK) articleMenu("?".TTH_PATH."=article_menu_edit", "edit", $article_menu_id, $category_id, $name, $slug, $title, $description, $keywords, $parent, $comment, $font_icon, $plus, $is_active, $hot, $img, $error);