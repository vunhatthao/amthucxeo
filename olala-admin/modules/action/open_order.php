...
if(isset($_POST['id'])) {
	...
	if($db->RowCount > 0) {
		foreach($rows as $row) {
			...
			elseif($row['icon']=='fa-globe')  $title = '[BOOK NOW]: ' . stripslashes($row['name']) . ' (' . $date->vnDateTime($row['created_time']) . ')';
			elseif($row['icon']=='fa-briefcase')  $title = '[ĐẶT TOUR]: ' . stripslashes($row['name']) . ' (' . $date->vnDateTime($row['created_time']) . ')';
			else $title = '[BOOK NOW]: ' . stripslashes($row['name']) . ' (' . $date->vnDateTime($row['created_time']) . ')';
